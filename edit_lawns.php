<?php
    ...
    {
        $id = $_GET['id'];
        $query = "SELECT * FROM `lawns_farmhouses` WHERE `id`='$id'";
...
<?php
    ...
    if(isset($_POST['submit']))
    {
        ...
        $query = "UPDATE `lawns_farmhouses` SET `name`='$name',`location`='$location',`address`='$address',`veg`='$veg',`non_veg`='$non_veg',`img`='$img',`indoor`='$indoor',`outdoor`='$outdoor',`in_out`='$in_out',`about`='$about' WHERE `id` = '$id_new'";
        ...
        {
            header("location:admin_lawns_farmhouses.php?update_msg=Data Updated Successfully...");
        }
    }
?>
...
<html lang="en">
    <head>
        ...
        <title>
        Update Lawns / Farmhouses
        </title>
        <link rel="shortcut icon" type="icon" href="img/wedmegood.png">
        <link rel="stylesheet" href="css/bootstrap.css">
    </head>
    <body>
        <div class="form-container-lawns">
            <form action="edit_lawns.php?id_new=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
                <h3>
                    Update Lawns / Farmhouses
                </h3>
                <label for="name">
                ...
                <input type="submit" name="submit" Value="Done" class="btn-update-lawns">
                <a href="admin_home.php" class="btn-back-lawns">
                    Back Home 
                </a>
            </form>
        </div>
